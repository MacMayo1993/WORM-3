...
- **Chaos rounds**: the wizard's cube (size, palette, style, scene, tunnels) is applied before Mobi's intro (`applyChaosSetup` in `useDisparityGame.js`). After the scramble the player aims the first strike (`chaosIgnitionPicking` / `chaosIgnition` in the store; taps route to the pick in `CubeAssembly`); the sim starts chain 0 on that tile by grid id (`ignition` in `createChaosSim`), and every later hop still uses fresh randomness. Chaos visuals go through `ChaosStorm` via the render-only `chaosStormBridge`; never route per-bolt events through Zustand.
