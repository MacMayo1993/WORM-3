...
The implementation checkpoint at the top records what is live and what remains planned. Cube/menu pads use `PadProvider` and a separate normal-offset transform. In cube modes and WORM, any live flipped face also raises its whole cubie to its Explode position; carried unflipped faces are platforms, not tunnel entries. Large tunnel anchors follow this finite piece movement, never the small pad’s continuous bounce. Stalk colors use the visible tile’s antipodal back color. WORM captures deliberate jumps onto raised faces via `raisedPlatforms.js`; unflipped carried faces are platforms. Fixed WORM pad height and expanded tunnel endpoints must agree with the simulation. Demo retains legacy crawl entry. Read the checkpoint and integration contracts before touching pad, tunnel-entry or rumble code.
