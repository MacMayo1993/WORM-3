...
import { BODY_BALL_SPACING, BASE_TAIL_LENGTH } from './constants.js';

export function stageWormPractice(sim, size, lesson) {
  resetWormSim(sim, size, { orbCount: 0, wormholeInterval: 9999 });
  const c = Math.floor(size / 2);
  const ring = ['surround', 'bomb'].includes(lesson.id);
  sim.pos = { x: ring ? c - 1 : c, y: ring ? c - 1 : 0, z: size - 1, dirKey: 'PZ' };
  ...
  if (lesson.id === 'heal') {
    ...
    sim.orbPickupColors = Array(2).fill('#a6eb9b');
  }
  ...
  return { cubies, inventory, target: marked, elapsed: 0, sawJump: false, jumps: 0, sawBoost: false, sawRocket: false, sawSignature: false, elementTime: 0 };
}
...
export function readWormPractice(sim, practice, lesson, state, size, delta) {
  practice.elapsed += Math.min(delta, 0.05);
  practice.sawJump ||= sim.isJumping;
  practice.jumps = Math.max(practice.jumps, sim.jumpCount);
  practice.sawBoost ||= sim.boostActiveT > 0;
  practice.sawRocket ||= sim.rocketActive;
  practice.sawSignature ||= sim.signature.seq > 0;
  ...
  switch (lesson.id) {
    ...
    case 'double-jump': done = practice.jumps >= 2 && !sim.isJumping; progress = `${practice.jumps} / 2 jumps`; break;
