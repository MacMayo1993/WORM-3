...
// Jump button — uses Red→Orange (faces 1↔4) antipodal gradient
const JUMP_BTN_BASE = {
    minWidth: 140,
    borderRadius: 14,
    padding: '18px 28px',
...
// Boost button — uses Yellow→White (faces 6↔3) antipodal gradient
...
// D-pad — compact cross shape, Blue (face 5) arrows
const DPAD_STYLE = {
    display: 'grid',
...
function BoostButton({ wormAlive, fc }) {
    ...
    const readyStyle = {
        ...BOOST_BTN_BASE,
        background: `linear-gradient(135deg, ${yellow}, ${fc[4] || FACE_FALLBACKS[4]})`,
        border: `1px solid ${yellow}`,
        boxShadow: `${SHADOW}, 0 0 12px ${yellow}`,
    };
    ...
    const activeStyle = {
        ...
        boxShadow: `${SHADOW}, 0 0 20px ${white}, 0 0 40px ${yellow}`,
    };
    ...
    return (
        ...
        >
            {boostState === 'cooldown' && (
                <div style={{ ...BOOST_FILL_STYLE, height: `${fillPct}%`, background: `${yellow}80` }} />
            )}
            <span style={{ position: 'relative', zIndex: 1 }}>⚡</span>
        </button>
    );
...
export default function WormCrawlerHUD({ phase, onFlippedTile, cubeSize: _cubeSize = 3, onHome, onSettings, onToggleAntipodal, antipodalActive = false, wormAlive = true, showDeathMenu = false, deathDetails = null, onRetry, onNewGame }) {
    ...
    const [showOrbDetail, setShowOrbDetail] = useState(false);
    ...
    // Auto-dismiss orb detail popover
    useEffect(() => {
        if (!showOrbDetail) return;
        const t = setTimeout(() => setShowOrbDetail(false), 3000);
    ...
    // Jump button — Red→Orange antipodal gradient
    const jumpReadyStyle = {
        ...JUMP_BTN_BASE,
        background: `linear-gradient(135deg, ${red}, ${orange})`,
        border: `1px solid ${jumpEdgeColor}`,
        boxShadow: `${SHADOW}, 0 0 12px ${jumpEdgeColor}, 0 0 24px ${jumpEdgeColor}`,
    };
        ...
        <div style={ROOT_STYLE}>
            ...
            <div style={GLANCE_STRIP_STYLE}>
                ...
                >
                    ...
                    {/* Orb detail popover */}
                    {showOrbDetail && (
                        <OrbInventoryHUD orbInventory={wormOrbInventory} faceColors={fc} compact />
                    )}
                </div>
            </div>
                    ...
                    <div style={DPAD_STYLE}>
                        {DPAD_DIRS.map(([label, dir, col, row]) => (
                            <button
                                ...
                                style={{ ...DPAD_BTN_BASE, gridColumn: col + 1, gridRow: row + 1, color: blue }}
                            >
                                {label}
                            </button>
                        ))}
