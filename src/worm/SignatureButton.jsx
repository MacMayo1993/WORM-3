...
export default function SignatureButton({ portalReady = false }) {
  ...
  const disabled = !alive || paused || !['active', 'finalHealing'].includes(phase) || !current || current.seconds > 0 || current.active;
  const label = current?.active ? 'ACTIVE' : current?.seconds > 0 ? `${current.seconds}s` : 'Q';
  ...
  return <div style={{ position: 'absolute', bottom: `calc(100% + ${portalReady ? 52 : 12}px)`, left: '50%', transform: 'translateX(-50%)', width: 'min(180px, 48vw)', textAlign: 'center' }}>
    <button type="button" disabled={disabled} onPointerDown={activate}
      ...
      aria-label={`${def.name}${current?.seconds > 0 ? `, ${current.seconds} seconds remaining` : ''}`}
      ...
      <span style={{ fontWeight: 800, fontSize: 12 }}>{def.short}</span>
