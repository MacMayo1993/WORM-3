...
export default function TunnelNeedsCard() {
  ...
  return <aside className="worm-tunnel-needs" aria-label="Tunnel healing requirements" style={{ color: GAME_HUD.text, background: GAME_HUD.raised, border: `1px solid ${need.ready ? '#a6eb9b' : '#ffffff35'}`, borderRadius: 16, padding: '10px 12px', pointerEvents: 'none', boxShadow: '0 6px 20px #0005' }}>
    <style>{`.worm-tunnel-needs{position:absolute;top:calc(env(safe-area-inset-top,0px) + 168px);left:50%;transform:translateX(-50%);width:min(290px,76vw)}@media(max-height:500px){.worm-tunnel-needs{top:calc(env(safe-area-inset-top,0px) + 80px);left:auto;right:12px;transform:none;width:210px}}`}</style>
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      <span aria-hidden="true" style={{ width: 30, height: 30, flexShrink: 0, borderRadius: '50%', background: need.isPrism ? 'conic-gradient(#ff739a,#ffdc72,#93e59c,#7cdaff,#b996ff,#ff739a)' : need.color, border: '2px solid #ffffffc0', boxShadow: `0 0 12px ${need.color}70`, display: 'grid', placeItems: 'center', color: '#16201a', fontWeight: 900 }}>{need.ready ? '✓' : need.pickupsNeeded}</span>
      <div>
    ...
    {!need.voided && <div role="progressbar" aria-label="Healing energy deposited" aria-valuetext={`${Math.round(need.savedFraction * 100)}% deposited, ${Math.round(need.payableFraction * 100)}% available from carried orbs`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(need.savedFraction * 100)} style={{ display: 'flex', overflow: 'hidden', height: 5, background: '#0005', borderRadius: 4, margin: '7px 0 5px' }}>
