...
const IntroTunnel = ({ start, end, color1, color2, opacity = 0.8, groupId: _groupId, formation = 1 }) => {
  ...
  useFrame((state, delta) => {
    ...
    // Surge signal - synced with wormhole energy pulses
    ...
    linesRef.current.forEach((line, i) => {
      ...
      if (line.material) {
        ...
        line.material.opacity = Math.min(1, config.baseOpacity * pulse * (1 + spark * 0.5) + surgeGlow) * opacity;
      }

      // Calculate midpoint and apply tug-of-war
      const midPoint = new THREE.Vector3().addVectors(_vStart, _vEnd).multiplyScalar(0.5);
      _controlPoint.copy(midPoint);
      ...
      for (let j = 0; j < points.length; j++) {
        ...
        if (distFromEnds < formation) {
          ...
          const endpoint = useStart ? _vStart : _vEnd;
      ...
      for (let j = 0; j < 30; j++) {
        ...
        // Brighten toward white for energy flash
        if (brightness > 0.01) {
