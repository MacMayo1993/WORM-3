...
export default function WormDemoLessonCard({ onRetry, onSkip }) {
  ...
  return <section className="worm-demo-card" aria-label="WORM practice">
    ...
    <div className="worm-demo-actions">
      <button ref={retryRef} disabled={!dead && !s.started && !s.prepared} className={!s.started && !dead ? 'worm-demo-next' : ''} onClick={!s.started && !dead ? s.start : onRetry ?? s.retry}>{dead ? 'Try again' : !s.started ? 'Try it' : 'Retry'}</button>
      <button className={s.complete ? 'worm-demo-next' : ''} onClick={s.next}>{s.complete ? (s.demoWormLessonIndex === WORM_DEMO_LESSONS.length - 1 ? 'Finish' : 'Next') : 'Skip exercise'}</button>
      <button onClick={onSkip ?? s.finish}>End practice</button>
    </div>
  </section>;
}
