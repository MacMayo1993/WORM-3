...
export default function MenuWormParticle({ start, character = 'classic', elapsed, arcPhase = 0, antipodal = false, delay = 0, onComplete }) {
  ...
  useFrame(() => {
    ...
    model.segments.forEach((_, i) => {
      const visible = sampleMenuTunnelWorm(path, distance - i * MENU_WORM_SPACING, pose.position, pose.normal, pose.forward);
      if (antipodal) { pose.position.negate(); pose.normal.negate(); pose.forward.negate(); }
