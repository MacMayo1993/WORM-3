...
it.each([3, 7, 15])('clears the actual raised cubie on every face of a %i cube', size => {
  for (const face of Object.keys(FACE_NORMALS)) {
    ...
    for (let i = 0; i <= 120; i++) {
      ...
      // A newly lowered 3x3 piece can initially overlap a worm already
      // underneath it. The sideways launch must escape, then never re-enter.
      const outside = delta.some(v => v >= .65);
      if (cleared) expect(outside).toBe(true);
      cleared ||= outside;
    }
