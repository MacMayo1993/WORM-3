...
beforeEach(()=>{
  set({playerProgress:newProgress(),xpRun:null,xpNotice:null,xpActivityRuns:{},parityPoints:100,ownedItems:['skin_slime','hat_none'],showMainMenu:false,showWelcome:false,showPlayerProgress:false,demoMode:false,teachModeActive:false,wormHealerMode:false,wormPaused:false,wormMissionsCompleted:0,currentLevel:null,currentLevelData:null,activePackId:'story-campaign',size:3,settings:{...store().settings,biomeMode:{enabled:false}},randomMode:false,chaosLevel:0,disparityRoundId:1,showDisparityWinner:false,disparityWinner:null,animState:null});
  store().resetGame();
});
...
describe('Worm XP at real run boundaries',()=>{
  ...
  it.each([3,7,15])('caps orb earnings on a size-%i cube',size=>{
    ...
    expect(store().playerProgress.xp).toBe(178);
    pickup(1001);pickup(4);pickup(NaN);expect(store().playerProgress.xp).toBe(178);
  });
  it('pays the same multiplier for batched and individual pickups',()=>{
    worm(2.75,10);for(let i=1;i<=80;i++)pickup(i);
    const amount=store().xpRun.xp;set({playerProgress:newProgress()});worm(2.75,10);pickup(80);
    expect(amount).toBe(154);expect(store().xpRun.xp).toBe(amount);
  });
  it('does not count a tunnel until the worm returns to crawling and pays a route once',()=>{
    ...
    expect(store().xpRun.xp).toBe(43);
    event('entry','2:5');event('tunnels');expect(store().xpRun.xp).toBe(51);
    event('entry','undefined:undefined');event('tunnels');expect(store().xpRun.xp).toBe(51);
  });
  it('pays the mission, its existing points and a crossed level atomically',()=>{
    ...
    expect(store().playerProgress.xp).toBe(105);
    expect(store().parityPoints).toBe(145); // 20 mission + 25 level
    expect(readPlayerSave()).toMatchObject({points:145,progress:{xp:105}});
    store().recordWormMission('orbs',8,1,id);expect(store().parityPoints).toBe(145);
    expect(store().xpRun.breakdown.Achievements).toBe(25);
  });
  it('limits healing and only pays the completed cube once',()=>{
    const id=worm();store().recordWormXp('healed',100,null,id);
    expect(store().xpRun.xp).toBe(150);store().finishWormXp(true,id);store().finishWormXp(true,id);
    expect(store().xpRun.xp).toBe(360);expect(store().xpRun.completed).toBe(true);
  });
...
describe('puzzle XP',()=>{
  it('pays first clear and stars once, then diminishing replay rewards',()=>{
    puzzle({level:1});solve();expect(store().xpRun.xp).toBe(225); // 30 solve + 40 first + 60 stars + 95 feats
    const p=store().playerProgress;store().setVictory('rubiks');expect(store().playerProgress).toBe(p);
    puzzle({level:1});solve();expect(store().xpRun.xp).toBe(38);
    puzzle({level:1});solve();expect(store().xpRun.xp).toBe(17);
  ...
  it.each(['freeplay','random','biome'])('rewards meaningful %s scrambles',mode=>{
    puzzle({mode});solve();expect(store().xpRun.mode).toBe(mode);expect(store().xpRun.xp).toBe(mode === 'freeplay' ? 170 : 230);
    expect(store().playerProgress.modeXp[mode]).toBe(mode === 'freeplay' ? 170 : 230);
  });
  ...
  it('a guided solve never consumes the independent first-clear and star rewards',()=>{
    ...
    puzzle({level:1});solve();expect(store().xpRun.xp).toBe(225);
  });
  it('rewards a daily once per puzzle date, and upgrades a guided clear fairly',()=>{
    ...
    puzzle(settings);solve();expect(store().xpRun.xp).toBe(220); // remaining 95 + 30 at par + 95 feats
    puzzle(settings);solve();expect(store().xpRun.xp).toBe(0);
    puzzle({...settings,daily:'2026-09-16'});solve();expect(store().xpRun.xp).toBe(225);
  });
});
...
describe('other release modes and claims',()=>{
  it('pays a completed Chaos round regardless of wager; skips aborts and duplicate results',()=>{
    ...
    expect(store().xpRun.xp).toBe(85);store().finishChaosXp();expect(store().xpRun.xp).toBe(85);
  });
  it('gives one-time lesson, quiz, exploration and introduction rewards',()=>{
    ...
    store().recordDiscoveryXp('cubelet');store().recordDiscoveryXp('cubelet');expect(store().playerProgress.xp).toBe(195);
    store().recordDiscoveryXp('introduction');expect(store().playerProgress.xp).toBe(195);
    set({demoMode:true,demoStep:'worm-traversal'});store().recordDiscoveryXp('introduction');store().recordDiscoveryXp('introduction');expect(store().playerProgress.xp).toBe(280);
  });
