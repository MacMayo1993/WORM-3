...
describe('progressManager singleton', () => {
  it('ties testMode to the build, never hardcoding it on', () => {
    ...
    // grinding; a shipped build must not have it unless this browser opted in
    // via ?unlockall=1, and Vite folds import.meta.env.DEV to false at bundle
    // time. Asserting the link here catches the flag being pinned to a literal
    // in either direction — which would either bypass progression for every
    // player or take the campaign away from dev.
    expect(progressManager.testMode).toBe(!!import.meta.env.DEV || UNLOCK_ALL);
  });
