...
export function wormXpChanges(state, kind, total, pair, runId) {
  ...
  if (kind === 'orbs' || kind === 'healed') {
    if (!Number.isSafeInteger(total) || total <= old) return {};
  ...
  return xpChanges(state, amount, 'worm', label, next);
}
...
export function puzzleXpChanges(state) {
  ...
  return { playerProgress: nextState.playerProgress, xpRun: nextState.xpRun, parityPoints: nextState.parityPoints, xpNotice: nextState.xpNotice };
}
