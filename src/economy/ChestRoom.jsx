    ...
    <div className="chest-page">
      ...
      <div className="chest-layout">
        <section className="chest-roll-panel" aria-label="Roll a chest">
          ...
          <div className="chest-options">{Object.entries(CHEST_MODES).map(([id, option]) => <button key={id} aria-pressed={mode === id} disabled={rolling} onClick={() => setMode(id)}><span className="chest-option-icon" aria-hidden="true">{id === 'single' ? '◇' : '◇ ◇'}</span><strong>{option.label}</strong><span>◆ {option.cost} gems</span></button>)}</div>
        ...
        <section className="chest-tiers" aria-label="Reward tiers"><div className="chest-tier-heading"><span className="chest-eyebrow">WHAT’S INSIDE</span><h2>THE SIX TIERS</h2><p>A little luck. A lot to collect.</p></div>{CHEST_TIERS.map((t, i) => ({ ...t, tier: i })).reverse().map(t => <button key={t.id} onClick={() => setPoolTier(t.tier)} aria-pressed={poolTier === t.tier} style={{ '--tier': t.color }}><i aria-hidden="true" /><span><strong>{t.name}</strong><small>{t.label}</small></span><b aria-hidden="true">›</b></button>)}<div className="chest-pool" aria-live="polite"><h3>{CHEST_TIERS[poolTier].name} rewards</h3>{poolTier === 0 ? <p>Every common result grants 25 PP and 25 XP.</p> : <><p>Unowned items have equal chances within this tier. When all are owned: {CHEST_TIERS[poolTier].compensation} gems.</p><ul>{chestPool(poolTier).map(item => <li key={item.id}>{item.label}{state.ownedItems.includes(item.id) ? ' · Owned' : ''}</li>)}</ul></>}</div></section>
      </div>
      ...
      <details className="chest-odds"><summary>Exact odds & rolling rules</summary><p>Each roll awards one reward. Paired cubies are independent and use stronger weights than single cubies. The final reward takes the lower tier, except matching colors move up one tier. Two mythic faces stay mythic.</p><div className="chest-table-scroll"><table><caption>Chance per roll, before choosing an item</caption><thead><tr><th>Tier</th><th>One cubie / final</th><th>Each paired cubie</th><th>Two cubies / final</th></tr></thead><tbody>{CHEST_TIERS.map((t, i) => <tr key={t.id}><th><i className="chest-tier-dot" style={{ background: t.color }} aria-hidden="true" />{t.name}</th><td>{percent(chestOdds('single')[i])}</td><td>{percent(CHEST_MODES.double.weights[i] / 10000)}</td><td>{percent(chestOdds('double')[i])}</td></tr>)}</tbody></table></div><p>Common gives PP and XP together; an XP level-up can also add its normal PP bonus. Other tiers first choose an unowned item uniformly; completed tiers give gems. Odds stay fixed between rolls. Roll costs are 10 and 15 gems respectively.</p></details>
