...
function rewardLabel(receipt) {
  ...
  return r.kind === 'choice' ? 'Choose your cosmetic' : r.kind === 'item' ? getStoreItem(r.itemId)?.label : r.kind === 'currency' ? `${r.points} Parity Points + ${r.xp} XP` : `${r.gems} gems · Tier collection complete`;
}
export default function ChestRoom({ onBack, onClose }) {
  ...
  return <div ref={root} onKeyDown={onKeyDown} className="chest-room" style={{ zIndex: Z.TOAST, fontFamily: UI_FONT, '--display-font': DISPLAY_FONT, '--paper': PAPER_SHEET, '--ink': PAPER_TEXT, '--muted': PAPER_TEXT_MUTED, '--line': PAPER_BORDER_SOFT, '--night': NIGHT_SHEET, '--night-ink': NIGHT_TEXT, '--night-muted': NIGHT_TEXT_MUTED, '--moss': UI_MOSS }} role="dialog" aria-modal="true" aria-labelledby="chest-title">
    <div className="chest-page">
      ...
      <div className="chest-masthead"><div><p className="chest-eyebrow">THE PARITY STORE / DISCOVER</p><h1 id="chest-title">CUBIE CHESTS</h1><p className="chest-intro">Six colors. One reward. Find your next favorite.</p></div><div className="chest-spectrum" aria-hidden="true">{[...CHEST_TIERS].reverse().map(t => <i key={t.id} style={{ background: t.color }} />)}</div></div>
      <div className="chest-layout">
        <section className="chest-roll-panel" aria-label="Roll a chest">
          <div className={`chest-arena${rolling ? ' is-rolling' : receipt ? ' is-revealed' : ''}`} key={`${receipt?.id ?? 'ready'}-${mode}`}
            ...
            <div className="chest-arena-caption"><span>THE ROLLING TABLE</span><span>{def.dice === 1 ? '01 CUBIE' : '02 CUBIES'}</span></div>
          ...
          <div className={`chest-result${!rolling && receipt ? ' is-revealed' : ''}`}  ref={resultRef} tabIndex={-1} role="status" aria-live="polite" style={{ '--tier': tier.color }}>
            {rolling ? <><strong>Rolling…</strong><p>Let’s see where they land.</p></> : receipt ? <><small><i aria-hidden="true" />Last reward · {tier.name}{receipt.faces.length === 2 && receipt.faces[0] === receipt.faces[1] ? receipt.faces[0] === 5 ? ' · MATCH' : ' · MATCH BONUS' : ''}</small><strong>{rewardLabel(receipt)}</strong><p>{receipt.faces.length === 2 ? receipt.faces[0] === receipt.faces[1] ? receipt.tier === 5 && receipt.faces[0] === 5 ? 'Double mythic! Mythic is the highest tier.' : 'Matching pair! Upgraded one tier.' : 'Different faces: the lower tier wins.' : 'Single cubie result.'} {choosing ? 'Pick your reward below.' : 'Reward added to your collection or wallet.'}</p></> : <><strong>Your first roll awaits</strong><p>Pick one cubie or try for a matching pair.</p></>}
          </div>
          {!rolling && choosing && <ChestRewardChoices receipt={receipt} ownedItems={state.ownedItems} onChoose={itemId => {
          ...
          <button className="chest-roll-button" disabled={rolling || choosing || state.demoMode || state.chestWallet.gems < def.cost} onClick={() => perform(() => state.roll(mode))}>{rolling ? 'Rolling…' : choosing ? 'Choose your reward above' : `Roll ${def.dice === 1 ? 'one cubie' : 'two cubies'} · ${def.cost} gems`}</button>
          {state.chestWallet.gems < def.cost && <p>Earn or exchange gems below to roll.</p>}
          {error && <p role="alert" className="chest-error">{error}</p>}
        </section>
        <section className="chest-tiers" aria-label="Reward tiers"><div className="chest-tier-heading"><span className="chest-eyebrow">WHAT’S INSIDE</span><h2>THE SIX TIERS</h2><p>A little luck. A lot to collect.</p></div>{CHEST_TIERS.map((t, i) => ({ ...t, tier: i })).reverse().map(t => <button key={t.id} onClick={() => setPoolTier(t.tier)} aria-pressed={poolTier === t.tier} style={{ '--tier': t.color }}><i aria-hidden="true" /><span><strong>{t.name}</strong><small>{t.label}</small></span><b aria-hidden="true">›</b></button>)}<div className="chest-pool" aria-live="polite"><h3>{CHEST_TIERS[poolTier].name} rewards</h3>{poolTier === 0 ? <p>Every common result grants 25 PP and 25 XP.</p> : <><p>Up to three distinct unowned items are offered at random. Choose one to keep. When all are owned: {CHEST_TIERS[poolTier].compensation} gems.</p><ul>{chestPool(poolTier).map(item => <li key={item.id}>{item.label}{state.ownedItems.includes(item.id) ? ' · Owned' : ''}</li>)}</ul></>}</div></section>
      </div>
