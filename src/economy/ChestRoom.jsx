      ...
      <div className="chest-layout">
        <section className="chest-roll-panel" aria-label="Roll a chest">
          ...
          <div className={`chest-result${!rolling && receipt ? ' is-revealed' : ''}`}  ref={resultRef} tabIndex={-1} role="status" aria-live="polite" style={{ '--tier': tier.color }}>
            {rolling ? <><strong>Rolling…</strong></> : receipt ? <><small><i aria-hidden="true" />Last reward · {tier.name}{receipt.faces.length === 2 && receipt.faces[0] === receipt.faces[1] ? receipt.faces[0] === 5 ? ' · MATCH' : ' · MATCH BONUS' : ''}</small><strong>{rewardLabel(receipt)}</strong><p>{receipt.faces.length === 2 ? receipt.faces[0] === receipt.faces[1] ? receipt.tier === 5 && receipt.faces[0] === 5 ? 'Double mythic! Mythic is the highest tier.' : 'Matching pair! Upgraded one tier.' : 'Different faces: the lower tier wins.' : 'Single cubie result.'} {choosing ? 'Pick your reward below.' : 'Reward added to your collection or wallet.'}</p></> : <><strong>Ready to roll</strong></>}
          </div>
        ...
        <section className="chest-tiers" aria-label="Reward tiers"><div className="chest-tier-heading"><h2>THE SIX TIERS</h2></div>{CHEST_TIERS.map((t, i) => ({ ...t, tier: i })).reverse().map(t => <button key={t.id} onClick={() => setPoolTier(t.tier)} aria-pressed={poolTier === t.tier} style={{ '--tier': t.color }}><i aria-hidden="true" /><span><strong>{t.name}</strong><small>{t.label}</small></span><b aria-hidden="true">›</b></button>)}<div className="chest-pool" aria-live="polite"><h3>{CHEST_TIERS[poolTier].name} rewards</h3>{poolTier === 0 ? <p>Common: 25 PP and 25 XP.</p> : <><p>Choose 1 of up to 3 unowned items. Tier complete: {CHEST_TIERS[poolTier].compensation} gems.</p><ul>{chestPool(poolTier).map(item => <li key={item.id}>{item.label}{state.ownedItems.includes(item.id) ? ' · Owned' : ''}</li>)}</ul></>}</div></section>
      </div>
