...
The route trial counts unique pair identities only after the head resumes crawling. A marker points to an unvisited pair, so revisiting a mouth cannot silently pad progress. Its four portals have no orb inventory and exclude ring healing to keep required routes available. The regular safe-traversal/void limits remain. The jump trial counts one clearance per continuous airborne episode only after an alive surface landing; a double jump does not add another count, and empty/low jumps or already severed body tiles do not count.
