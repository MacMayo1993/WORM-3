...
8. ✅ WormBody tail LOD + color-epoch updates (§2.3): color-epoch (`colorEpochRef`-gated `setColorAt`/`needsUpdate`, skipped when nothing changed) plus distance-LOD on the tail — every segment rendered within 200 of the head, every 2nd segment from 200-600, every 4th beyond 600, with the rendered segment scaled up by `lodStep` to fill the gap. Skipped logical indices `continue` out of the curve-walk entirely (safe because `targetDist` is a pure function of the logical index `i`, so the walk's cumulative distance naturally catches up), saving the per-segment curve-walk cost, not just the GPU write. GPU instance slots are written through a separate compacted `writeIdx` counter (decoupled from the logical `i`) since `InstancedMesh` slots must be contiguous; `mesh.count` is set to `writeIdx` after the loop. Glow-worm's alternating overlay segments and inch-worm's alternating body/belly bands were re-keyed off `writeIdx` instead of `i` so the cosmetic alternation ratio survives LOD thinning (under the old `i`-based check, `i % 2` stops alternating once `lodStep` itself is even). Cuts worst case from 1,200 to ~450 writes. The `lodStep` gap-fill scale-up is a linear approximation (segments are ~touching spheres, so spacing × N needs diameter × N) not yet visually confirmed in-browser; only engages at tail lengths past 200 segments (many orbs eaten in one session), so exposure in normal play is low.
...
10. ✅ `WormholeRings`' cubies debounce (§2.2) is phase-aware: 400 ms during `crawling` (rings already throttle to 20 Hz there) vs. 200 ms during `entering`/`tunnel`/`exiting` for the 60 Hz ring cadence. **Plus the deeper option**: `buildManifoldGridMapIncremental` (`src/game/manifoldLogic.js`) replaces the full `O(size³×6)` map rebuild with a cached diff — a `{ map, prevCubies, size }` cache held in a `useRef` is compared cell-by-cell by reference against the previous call, and only cells whose cubie object actually changed get their 6 sticker gridId entries recomputed. Correct by construction: gridId is derived from a sticker's immutable `orig`/`origPos`/`origDir`, and the codebase's shallow-clone-with-shared-refs pattern (`flipStickerPair`/`rotateSliceCubies`) guarantees only genuinely-changed cells get new object references, so unchanged cells' entries (and thus the whole map) stay byte-for-byte identical to what a full rebuild would produce — verified directly in `src/__tests__/manifoldLogic.test.js` across flip-only, rotation-only, and interleaved sequences. Wired into both call sites that matter: `WormholeRings`' debounced `allPositions` memo, and — the actually-hotter, non-debounced path — the `tunnelLookupRef` effect that reruns on every raw `cubies` change (~12×/sec at chaos L4) to keep tunnel-entry lookup exact for gameplay. `getActiveTunnels`' own `O(size³)` flipped-sticker scan is unchanged (still needs the full picture each time); true `O(1)`-per-flip tunnel-list maintenance would require hooking flip events at the source and was judged too high-risk for a core gameplay mechanic to attempt without playtest access — left as a future option if profiling shows the manifold-map share of the cost wasn't enough.
...
12. ✅ Adaptive quality tier: `App.jsx` renders drei's `<PerformanceMonitor>` inside the `<Canvas>`; on a sustained decline (rolling-average fps below bounds over ~2.5 s) it clamps `dpr` from `[1, 1.5]` down to `[0.75, 1]` and sets a new `perfReducedFX` flag in the Zustand store, restoring both on a sustained incline. `StickerPlane.jsx`'s existing `suppressVolumeFX` fallback (previously gated only on cube size ≥ 6) now also drops on `perfReducedFX`, so sizes 3–5 get the same flat-shader fallback automatically on underpowered devices instead of only at size 6–7. Not yet visually confirmed in-browser that the decline/incline thresholds (drei's defaults: 10 samples × 250 ms, threshold 0.75) feel responsive rather than flappy at actual gameplay framerates.
