...
extension (§18) replaces the free involution by a free action of `(Z₂)^r`
generated by commuting involutions,
...
weighted word length connecting two states. The one-involution model factors all
the way down to individual syndrome *coordinates*. Under the `(Z₂)^r` extension of
§18 those coordinates couple *within* each `G`-orbit while orbitwise
factorisation survives; genuine *inter-orbit* coupling is introduced only in §19.
...
distinction is constructive rather than defensive: it marks where *cochain*
coupling begins (§18, within each orbit) as opposed to where genuinely
nonlocal, inter-orbit decoding can begin (§19).
...
`τᵢτⱼ = τⱼτᵢ`). **Assume the action is free** (hence faithful, on a nonempty set).
This is a real hypothesis, not a formality: `r` individually fixed-point-free
commuting involutions need not generate a free `(Z₂)^r`-action with
`2^r`-element orbits — a product `τᵢτⱼ` may have fixed points, or the
...
> `G`-invariant sector `Δ = 0` requires `b` constant on each orbit. With
> unit-cost single-member toggles, the minimum cost factorises over orbits:
> ```
> C_sector = Σ_{orbits o} min( k_o , 2^r − k_o ),     k_o = wt(b |_o).
> ```
> *Proof.* `Δ|_o = 0` iff `b` is constant on orbit `o`, i.e. the invariant target
> *states* on orbit `o` are exactly `ker δ⁰|_o = {0, 1}`. Single-member toggles
> reach the all-`0` target at cost `k_o` and the all-`1` target at cost
> `2^r − k_o`, and no non-constant state has `Δ = 0`, so the per-orbit minimum is
> `min(k_o, 2^r − k_o)`. Orbits are disjoint, so the sum is achievable. ∎
...
Problem (3) is the genuine coding problem, but its difficulty has **two distinct
sources that must not be conflated** — `H¹ ≠ 0` is *not* the exact criterion:

```
topological obstruction:      Z¹ / B¹ = H¹ ≠ 0     (closed but non-exact fields exist)
measurement/noise inconsistency:  η ∉ Z¹            (the observed field is not even closed)
```

When `H¹ = 0` we have `Z¹ = B¹`, so *every closed field is exact* — but an
arbitrary observed `η` with `δ¹η ≠ 0` is not closed, and
`min_{β∈B¹} d(η, β)` can still be a genuine nearest-codeword problem. On a
1-skeleton, `B¹ = im δ⁰` is the cut/coboundary space, and finding the closest
coboundary to noisy edge observations is a signed-graph frustration / signed-cut
optimisation — nontrivial independently of the higher-cell topology. So the three
regimes are:

- **A — exact:** `η = δ⁰ b`. Integration reconstructs `b` up to per-orbit
  constants (Theorem 5). Closed-form.
- **B — noisy, topologically trivial complex:** `η ∈ C¹`, `δ¹η ≠ 0`, `H¹ = 0`.
  No cohomological obstruction once closure is achieved, but nearest-exact
  decoding is already a nontrivial (signed-cut-type) problem.
- **C — nontrivial topology:** `η ∈ Z¹`, `[η] ≠ 0 ∈ H¹`. Locally consistent yet
  globally non-integrable — a distinct, purely topological difficulty.

Regime B is *local inconsistency*; regime C is a *global topological obstruction*.
This cocycle-versus-coboundary separation is the same device homological code
constructions use to tell trivial cycles from logical classes (Breuckmann–Terhal
...
- **The easy decoder comes from factorisation, and it degrades in two stages.**
  ...
  solvable by integration** (Theorem 5). Two further mechanisms must be kept
  distinct: *independent edge noise* (§18.5 regime B) makes each orbit's
  nearest-exact problem harder while **preserving** the orbitwise sum
  `C_total = Σ_o C_o`; only *gluing / cross-orbit constraints* (§19) remove the
  product structure itself and can make decoding genuinely nonlocal. The
  simplicity is a theorem about which decomposition holds, not evidence that
  coupled topological decoding is easy.
...
- Watson, G. S. (1966). The statistics of orientation data. *The Journal of
  Geology*, 74(5), 786–797. — orientation/axial data on the sphere.
