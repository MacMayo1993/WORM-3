...
- Use `HEADING_FONT` for interface headings, `UI_FONT` for controls and copy, and `DISPLAY_FONT` only for the game brand and large arcade stamps. Use sentence case for interface titles and actions.
...
- Name actions plainly: Next, Play, Resume, Replay, Equip. Show a price or an irreversible consequence beside the action when it changes the decision.
